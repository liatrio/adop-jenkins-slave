pipeline {
    ...
    stages {
            ...
            steps {
                ...
                script {
                    if ( env.BRANCH_NAME == 'master' ) {
                        containerVersion = getVersionFromContainer("liatrio/ldop-jenkins-slave:${env.BRANCH_NAME}")
                        failIfVersionExists("liatrio","ldop-jenkins-slave",containerVersion)
                        sh "docker build -t liatrio/ldop-jenkins-slave:${containerVersion} ."
                    }
                }
            }
            post {
        ...
        stage('Push to dockerhub') {
            ...
            steps {
                script { CHANGED = "NO" }
                sh "docker tag liatrio/ldop-jenkins-slave:${env.BRANCH_NAME} liatrio/ldop-jenkins-slave:latest"
                sh "docker push liatrio/ldop-jenkins-slave:latest"
                script {
                    if ( env.BRANCH_NAME == 'master' )
                        sh "docker push liatrio/ldop-jenkins-slave:${containerVersion}"
                }
            }
            post {
                success { script { STATUS = "SUCCESS" } }
